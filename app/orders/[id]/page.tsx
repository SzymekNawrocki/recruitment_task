...
export default async function OrderDetailPage({
  ...
  const isLocked = order.status === 'APPROVED' || order.status === 'CANCELLED' || order.status === 'REJECTED'
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-gray-400 text-xs uppercase tracking-wide mb-0.5">Dział</p>
            <p className="font-medium">{order.department}</p>
          </div>
          <div>
            <p className="text-gray-400 text-xs uppercase tracking-wide mb-0.5">Priorytet</p>
            <span
          ...
          <div>
            <p className="text-gray-400 text-xs uppercase tracking-wide mb-0.5">Data złożenia</p>
            <p className="font-medium">
              {new Date(order.createdAt).toLocaleDateString('pl-PL')}
            </p>
          </div>
          <div className="col-span-2">
            <p className="text-gray-400 text-xs uppercase tracking-wide mb-0.5">Uzasadnienie</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-gray-600">Przedmiot</th>
              <th className="text-right px-4 py-3 font-medium text-gray-600">Cena jedn.</th>
              <th className="text-right px-4 py-3 font-medium text-gray-600">Ilość</th>
              <th className="text-right px-4 py-3 font-medium text-gray-600">Razem</th>
            </tr>
          </thead>

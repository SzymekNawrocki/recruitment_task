import type { Dict } from './pl'

const en: Dict = {
  nav: {
    orders: 'Orders',
    newOrder: 'New Order',
    admin: 'Admin Panel',
  },
  orders: {
    title: 'Orders',
    newButton: '+ New Order',
    empty: 'No orders yet',
    emptyLink: 'Place the first order',
    cols: {
      employee: 'Employee',
      department: 'Department',
      priority: 'Priority',
      value: 'Value',
      status: 'Status',
      date: 'Date',
    },
  },
  newOrder: { title: 'New Order' },
  editOrder: { title: 'Edit Order' },
  detail: {
    back: '← Orders',
    edit: 'Edit',
    cancel: 'Cancel Order',
    cancelConfirm: 'Cancel this order?',
    cancelling: 'Cancelling…',
    department: 'Department',
    priority: 'Priority',
    status: 'Status',
    date: 'Submitted',
    justification: 'Justification',
    cols: {
      item: 'Item',
      unitPrice: 'Unit price',
      qty: 'Qty',
      total: 'Total',
    },
    total: 'Total',
  },
  form: {
    sectionInfo: 'Requestor details',
    sectionCatalog: 'Equipment catalogue',
    sectionItems: 'Selected items',
    employeeName: 'Full name',
    employeePlaceholder: 'Jane Smith',
    department: 'Department',
    departmentPlaceholder: 'e.g. IT, HR, Finance',
    justification: 'Justification',
    justificationPlaceholder: 'Describe the business need...',
    priority: 'Priority',
    subtotal: 'Total',
    overBudget: 'Exceeds the 5,000 PLN limit. Set priority to "High".',
    perUnit: 'PLN/ea.',
    cancel: 'Cancel',
    submit: 'Place Order',
    save: 'Save Changes',
    saving: 'Saving…',
  },
  admin: {
    title: 'Admin Panel',
    empty: 'No pending orders',
    emptyLink: 'View all orders',
    approve: 'Approve',
    reject: 'Reject',
    rejectConfirm: 'Reject this order?',
    items: 'items',
  },
  catalog: {
    categories: {
      Peryferia: 'Peripherals',
      Monitory: 'Monitors',
      Audio: 'Audio',
      Akcesoria: 'Accessories',
      Ergonomia: 'Ergonomics',
      Zasilanie: 'Power',
    },
    items: {
      'Mysz optyczna Logitech MX Master 3': 'Logitech MX Master 3 Optical Mouse',
      'Klawiatura mechaniczna Keychron K2': 'Keychron K2 Mechanical Keyboard',
      'Monitor 24" Full HD Dell': 'Dell 24" Full HD Monitor',
      'Monitor 27" 4K LG': 'LG 27" 4K Monitor',
      'Słuchawki z mikrofonem Sony WH-1000XM5': 'Sony WH-1000XM5 Headset',
      'Stacja dokująca USB-C Anker 13-in-1': 'Anker 13-in-1 USB-C Docking Station',
      'Kamera internetowa Logitech C920 HD': 'Logitech C920 HD Webcam',
      'Podstawka pod laptopa Nexstand K2': 'Nexstand K2 Laptop Stand',
      'Podkładka pod mysz XL': 'XL Mouse Pad',
      'Hub USB 4-portowy': '4-Port USB Hub',
      'Kabel HDMI 2m': 'HDMI Cable 2m',
      'Zasilacz UPS 650VA': '650VA UPS Power Supply',
    },
  },
  priority: {
    low: 'Low',
    medium: 'Medium',
    high: 'High',
  },
  status: {
    PENDING: 'Pending',
    APPROVED: 'Approved',
    REJECTED: 'Rejected',
    CANCELLED: 'Cancelled',
  },
  app: {
    title: 'IT Portal',
    description: 'Internal IT equipment ordering portal',
  },
  errors: {
    employeeNameRequired: 'Full name is required',
    departmentRequired: 'Department is required',
    justificationRequired: 'Justification is required',
    invalidPriority: 'Priority must be: low, medium or high',
    itemsRequired: 'Order must contain at least one item',
    invalidQuantity: 'Item quantity must be an integer between 1 and 20',
    overBudgetDetail:
      'Order total ({total} PLN) exceeds {max} PLN. Change priority to "high" or reduce the order.',
    orderNotFound: 'Order not found',
    orderApprovedNoEdit: 'Approved order cannot be edited',
    orderApprovedNoCancel: 'Approved order cannot be cancelled',
    orderAlreadyCancelled: 'Order is already cancelled',
    onlyPendingApprove: 'Only pending orders can be approved',
    onlyPendingReject: 'Only pending orders can be rejected',
  },
}

export default en
